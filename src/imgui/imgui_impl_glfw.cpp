...
{
    // GFX PATCH: Changed to capture mouse
    if (g_PrevUserCallbackMousebutton != NULL && !ImGui::GetIO().WantCaptureMouse)
...
{
    // GFX PATCH: Changed to capture mouse
    if (g_PrevUserCallbackScroll != NULL && !ImGui::GetIO().WantCaptureMouse)
...
{
    // GFX PATCH: Changed to capture mouse
    if (g_PrevUserCallbackKey != NULL && !ImGui::GetIO().WantCaptureKeyboard)
...
{
    ImGuiIO& io = ImGui::GetIO();
    if (g_PrevUserCallbackChar != NULL && !io.WantTextInput)
